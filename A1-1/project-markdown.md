...
텍스트 생성을 위한 정교한 지시문부터 이미지 생성용 키워드, 복잡한 페르소나 설계안까지. 하지만 정작 필요할 때 “그때 그 프롬프트 어디 갔지?” 하며 메모장과 노션, 메신저 창을 뒤적였던 경험 또한 한 번쯤은 있으셨을 겁니다.
...
- `init`, `add`, `commit`, `push`, `pull`, `checkout`, `clone`, `merge`를 각각 1회 이상 사용해야 한다.
- 브랜치 생성 및 병합은 로컬에서 `checkout`과 `merge`로 수행 기록을 남긴다.
