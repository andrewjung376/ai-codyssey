...
여행 날짜를 입력하면 LLM(OpenAI)이 여행지 2~3곳을 추천하고, Kakao Local API로 각 지역의 맛집을 검색한 뒤, 결과를 종합해 지역별 최종 여행 리포트(Markdown)를 생성하는 CLI 프로그램이다.

## 개요

1. **[1/3]** OpenAI에 여행 날짜를 입력해 추천 지역 2~3곳(각각 날씨/행사/추천 이유 포함)을 JSON으로 받는다.
2. **[2/3]** 추천된 지역마다 Kakao Local API에서 맛집 5곳을 검색한다.
3. **[3/3]** 1·2단계 결과를 종합해 OpenAI가 지역별로 구분된 최종 여행 리포트를 Markdown으로 생성한다.
...
출력: `recommended_cities` 배열(2~3개)을 가진 JSON. 각 항목은 `city`/`weather`/`events`/`reason` 4개 키를 가진다.
...
```json
{
  "recommended_cities": [
    ...
    {
      "city": "강릉",
      "weather": "3월 중순 평균 10°C 내외, 일교차가 큼",
      "events": ["강릉 벚꽃축제"],
      "reason": "동해안의 봄바다와 커피거리를 함께 즐기기 좋은 시기입니다."
    }
  ]
}
```
...
- **복수 지역 처리**: `recommended_cities` 배열을 순회하며 지역마다 `search_restaurants()`를 호출하고, `{city: [맛집, ...]}` 형태의 딕셔너리로 모은다.

입력: 1단계 출력의 `recommended_cities` 배열 (예: `["제주", "강릉"]`)

출력: 지역명을 키로 하는 맛집 딕셔너리. 각 값은 맛집 최대 5곳의 리스트(0건이면 빈 리스트 `[]`)
...
입력: 1단계 JSON(`recommended_cities`) + 2단계 지역별 맛집 딕셔너리(지역마다 0건일 수 있음)

출력: 지역별로 구분된 Markdown 텍스트. 지역 수만큼 `### {지역명}` 소제목이 반복된다.
...
같은 `-date`로 다시 실행하면 이미 저장된 원본 JSON(`results/{date}_raw_data.json`)을 재사용해 1·2단계(LLM 추천, Kakao 맛집 검색) API 호출을 건너뛰고, 리포트만 새로 생성한다.
...
```
[캐시 발견] results\2026-03-15_raw_data.json 를 재사용합니다. 1~2단계(LLM 추천/맛집 검색) API 호출을 건너뜁니다.
  - recommended_cities: ['제주', '부산', '경주']
[3/3] 최종 리포트 생성 중(LLM)...
  - 리포트 생성 완료
...
새로 조회하고 싶다면 해당 날짜의 `results/{date}_raw_data.json`을 삭제한 뒤 다시 실행한다.
...
| 파일 | 내용 |
|---|---|
| `results/{date}_raw_data.json` | 지역별 1차 추천 JSON(`recommendation.recommended_cities`), 지역별 맛집 검색 결과(`restaurants`), 오류 요약(`errors`) |
| `results/{date}_travel_plan.md` | 지역별 추천 이유, 날씨, 행사, 맛집, 1일 일정과 오류 요약이 포함된 최종 리포트 |

맛집 검색이 0건이거나 Kakao API 인증(401/403) 오류가 발생해도 프로그램은 중단되지 않고, 해당 지역의 맛집 섹션만 "데이터 없음"으로 표기한 뒤 다른 지역과 리포트 생성은 계속 진행한다.
...
- **케이스 2(JSON 파싱 실패 재시도)**: 1차 요청에서 의도적으로 다른 스키마(`city`, `temp`)를 요구해 OpenAI가 `recommended_cities` 없는 JSON을 반환하게 한 뒤, `travel_planner.py`의 재시도 프롬프트(`build_recommendation_prompt(date, retry=True)`)로 2차 요청을 보내 `validate_recommendation()`을 통과하는(지역 2~3곳, 각 항목이 `city`/`weather`/`events`/`reason`을 가진) JSON을 받는지 확인한다.

두 케이스 모두 `PASS` 문구가 출력되면 정상이며, `assert` 실패 시 예외가 발생한다. `travel_planner.py`의 프로덕션 코드는 수정하지 않고, 해당 파일의 함수를 그대로 불러와 검증한다.

### 3. 결과 캐싱 (복수 지역 추천)

같은 `-date`로 두 번 연속 실행해 두 번째 실행이 1·2단계 API 호출 없이 캐시를 재사용하는지 확인한다.

```bash
python travel_planner.py -date "2026-05-01"
python travel_planner.py -date "2026-05-01"
```

두 번째 실행에서 `[캐시 발견] ... API 호출을 건너뜁니다.` 로그가 출력되고, `[1/3]`/`[2/3]` 단계 없이 바로 `[3/3]`으로 진행되는지 확인한다.
